      ...
      <nav ref={navRef} className="flex justify-between items-center max-container">
        ...
        <ul className="hidden lg:flex lg:text-sm lg:gap-5 flex-1 justify-center items-center gap-10">
          ...
          <li className="flex gap-4">
            {isAuthenticated ? (
              ...
              <>
                <Button label="Login" onClick={() => navigate("/login")} textColor="text-white" />
                <Button label="Signup" onClick={() => navigate("/signup")} textColor="text-white" />
              </>
            )}
          </li>
        </ul>

        {/* Cart and Auth Buttons for Desktop */}
        ...
        {isNavOpen && (
          ...
          >
            <button
              onClick={toggleNav}
              className="text-xl text-coral-red hover:text-red-600 font-bold mb-4 self-end"
            >
              ×
            </button>
